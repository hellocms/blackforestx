...
const CreateStockEntry = () => {
  ...
  useEffect(() => {
    const fetchDealers = async () => {
      try {
        const response = await fetch('https://apib.theblackforestcakes.com/api/dealers', {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(`https://apib.theblackforestcakes.com/api/dealer/products?category=${selectedCategory}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (response.ok) {
          setProducts(result);
          form.setFieldsValue({ product: null }); // Reset product selection when category changes
        } else {
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await fetch('https://apib.theblackforestcakes.com/api/dealer/stock-entries', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        message.success(result.message || 'Stock entry created successfully');
        form.resetFields();
      ...
      message.error('Server error while creating stock entry');
      console.error('Error:', err);
    } finally {
      setLoading(false);
    }
      ...
      <Row justify="center" align="middle" style={{ minHeight: '100vh' }}>
        <Col xs={24} sm={20} md={16} lg={12}>
          ...
          <Form
            ...
            initialValues={{ dealer: null, category: null, product: null, pcs_count: '', amount: '' }}
              ...
              >
                {dealers.map((dealer) => (
                  <Option key={dealer._id} value={dealer._id}>
                    {dealer.dealer_name} {/* Changed from dealer.name to dealer.dealer_name */}
                  </Option>
                ))}
              </Select>
            </Form.Item>
...
CreateStockEntry.useLayout = false;
