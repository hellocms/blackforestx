...
const StockEntryList = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch('https://apib.theblackforestcakes.com/api/dealer/products', {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      });
      const result = await response.json();
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`https://apib.theblackforestcakes.com/api/dealer/stock-entries/${id}`, {
        method: 'DELETE',
      });
      const result = await response.json();
      if (response.ok) {
