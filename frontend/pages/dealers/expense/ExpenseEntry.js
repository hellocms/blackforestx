...
const ExpenseEntry = () => {
  ...
  const fetchBills = async () => {
    setLoading(true);
    try {
      const response = await fetch('https://apib.theblackforestcakes.com/api/dealers/bills', {
        method: 'GET',
  ...
  const handleSave = async (bill) => {
    ...
    try {
      const response = await fetch(`https://apib.theblackforestcakes.com/api/dealers/bills/${bill._id}`, {
        method: 'PUT',
        body: formData,
      });
      const result = await response.json();
