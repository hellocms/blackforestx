...
const FinancialManagement = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter transactions based on branch for non-Office users
        if (!isOfficeBranch && branchId) {
          setFilteredTransactions(formattedTransactions.filter((t) => t.branchId === branchId));
  ...
  const handleDeposit = async (values) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!isOfficeBranch && branchId && newTransaction.branchId === branchId) {
          setFilteredTransactions((prev) => [...prev, newTransaction]);
        } else if (isOfficeBranch) {
          setFilteredTransactions((prev) => [...prev, newTransaction]);
        }
  ...
  const filterTransactions = (source, branch, type, dateRange) => {
    ...
    if (branch && !isOfficeBranch) {
      filtered = filtered.filter((t) => t.branchId === branch);
    } else if (branch && isOfficeBranch) {
      filtered = filtered.filter((t) => t.branchId === branch);
    }
  ...
  const clearFilters = () => {
    ...
    if (!isOfficeBranch && branchId) {
      setFilteredTransactions(transactions.filter((t) => t.branchId === branchId));
    } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (decoded.branchId) {
        ...
        fetchBranches(); // Fetch all branches for Office branch
      } else {
